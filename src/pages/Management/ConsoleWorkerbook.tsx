...
const ConsoleWorkerbook: React.FC = () => {
  ...
  const renderMiniCalendar = (
    ...
    return (
      <div className="bg-gray-800 rounded-lg p-4 w-full max-w-xs space-y-4">
  ...
  const renderToday = () => {
    ...
    const averageEQ =
      showedWorkers.length > 0 ? totalEquivalent / showedWorkers.length : 0;
    const cartAverage =
    ...
    const attendanceContent = (
      <div
        className={`h-[calc(100vh-17rem)] flex ${
          isTeamSeason ? 'bg-black' : 'bg-[#1a2832]'
        }`}
      >
        {isTeamSeason && (
          <div className="w-1/2 bg-gray-900/50 p-4 space-y-4 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
        ...
        <div
          className={
            isTeamSeason
              ? 'w-1/2 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800'
              : 'flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800'
          }
    ...
    const payoutContent = (
      <div className="px-6 overflow-y-auto h-[calc(100vh-17rem)] scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
        {isTeamSeason ? (
          // Team Payout View
                ...
                <div
                  ...
                  onClick={
                    member.payoutCompleted
                      ? () =>
                          navigate(
                            `../payout/contractor/${member.contractorId}`
                          )
                      : undefined
                  }
  ...
  const renderNextDay = () => {
    const notConfirmedWorkers = nextDayWorkers.filter(
      (w) => !w.confirmationStatus?.confirmed
    );
    const confirmedWorkers = nextDayWorkers.filter(
      (w) => w.confirmationStatus?.confirmed
    );

    return (
      <div className="px-6 h-[calc(100vh-13rem)] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
        ...
        {notBookedWorkers.map((member) => (
          ...
          >
            <input
              ...
              onChange={() => toggleWorkerSelection(member.contractorId)}
  ...
  const TABS_TO_RENDER = TABS.filter(
    (tab) =>
      !isPreSeason ||
